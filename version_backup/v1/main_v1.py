...
def score_to_clo(clo_file):
    ...
    # sum the number of CLO for each activity
    clo_sum = np.sum(clo_map, axis=1)
    # Elementwise divide score to number of CLO the score contribute to
    score_share = score/clo_sum
    # Calculate overall score contribution to each CLO
    out_score_to_clo = np.dot(score_share,clo_map)
...
def score_to_plo(clo_file):
    ...
    course_name = clo_file[3:clo_file.find('.')] # get course name from file name
    ...
    # sum the number of PLO for each CLO
    score_share = score_clo/np.sum(plo_map, axis=1) # calculate %share of each PLO will get contribution from CLO
    # Example a CLO that contribute 40% to the course map to PLO1 and PLO2, so each PLO will get 20% share
    score_to_plo = np.dot(score_share,plo_map)
...
# Function to map letter grades to numerical values
...
for file in student_files:
    ...
    grade_df = student_df.iloc[:,1:].applymap(map_grades) ## can be substituted with grade_df = student_df.iloc[:,1:].map(map_grades)
    ...
    for i in range(len(grade_df)): # Looping through student i
        ...
        df_temp = grade_df.iloc[i,:] # get percent score for each course
        # get the name (or id) of courses that student i has grade (registered)
        courses = grade_df.columns[df_temp.notna()]
        # get grading score of courses that student i has grade (registered)
        ...
        total_selected_plo = selected_plo.sum() # sum scores over row (produce total PLO score of registered courses of student i)
        # dot product of student i grading score with plo weight score give score of student i on each PLO
        ...
        # calculate percent student get from all registered PLO score
...
df = students_plo_score_df.copy()  # students_plo_score_df  or students_plo_percent_df
# Calculate percent compare to maximu score of each PLO
max_overall = df.max()
df = (df/max_overall) * 100
